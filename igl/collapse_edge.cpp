...
#include <vector>
#include "C:\Users\rotem\Documents\cmake\EngineForAnimationCourse\igl\circulation.h"
    ...
    int& a_f2)
{
    ...
    const std::vector<int> nV2Fd = circulation(e, eflip == 0, EMAP, EF, EI);
    ...
    std::vector<Eigen::Matrix4d> &Verror)
{
    ...
    return
        collapse_edge(
            cost_and_placement, always_try, never_care,
            V, F, E, EMAP, EF, EI, Q, Qit, C, Verror, e, e1, e2, f1, f2);
}
    ...
    int& f2)
{
    ...
    N.insert(N.begin(), Nd.begin(), Nd.end());
    bool collapsed = true;
    if (pre_collapse(V, F, E, EMAP, EF, EI, Q, Qit, C, Verror, e))
    {
        collapsed = igl::collapse_edge(e, C.row(e), V, F, E, EMAP, EF, EI, Verror, e1, e2, f1, f2);
    }
    else
    {
        // Aborted by pre collapse callback
    ...
    double costprint=0;
    Eigen::Vector3d pprint=Eigen::Vector3d(8888,8888,8888);
    post_collapse(V, F, E, EMAP, EF, EI, Q, Qit, C, Verror, e, e1, e2, f1, f2, collapsed);
    if (collapsed)
    {
        // Erase the two, other collapsed edges
        Q.erase(Qit[e1]);
        ...
        for (auto n : N)
        {
                ...
                F(n, 2) != IGL_COLLAPSE_EDGE_NULL)
            {
                for (int v = 0; v < 3; v++)
                {
                    ...
                    double cost;
                    RowVectorXd place;
                    cost_and_placement(ei, V, F, E, EMAP, EF, EI, cost, place, Verror);
                    // Replace in queue
                    costprint = cost;
                    pprint = place;
                    Qit[ei] = Q.insert(std::pair<double, int>(cost, ei)).first;
    ...
    std::cout << "edge " << e << " cost = " << costprint << " new v position (" << pprint[0] << "," << pprint[1] << "," << pprint[2] << ")\n";
    return collapsed;
}
            ...
            int& a_f2)
        {
            ...
            // The following implementation strongly relies on s<d
            assert(s < d && "s should be less than d");
            // move source and destination to midpoint, fix Verror
            V.row(s) = p;
            V.row(d) = p;
            Eigen::Matrix4d sum = Verror[s] + Verror[d];
            Verror[s] = sum;
            Verror[d] = sum;
